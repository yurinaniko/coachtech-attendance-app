...
- phpMyAdmin（DB確認用）
  http://localhost:8081
- MySQL はホスト側 3307 番で公開（GUIツールから接続する場合）
...
本アプリでは管理者も一般ユーザーとして勤怠打刻および修正申請を行うことができ、
管理者権限により他ユーザーおよび自身の申請の承認が可能な仕様としています。
...
Docker 環境上でテストを実行し、
全 83 テスト（Feature 14ファイル＋Unit 1ファイル）が PASS することを確認しています。
CI（GitHub Actions）でもプッシュごとにテスト実行と composer audit --locked による
依存脆弱性監査（厳格モード）を行っています。

## 使用技術

- PHP 8.3
- Laravel 12.x（Laravel 8で構築後、8→9→10→11→12へ段階的にアップグレード済み。各段階でテスト緑・依存脆弱性0件を確認）
...
## 工夫した点
- バリデーションエラー時の入力保持（old優先）を実装
- 動的な休憩入力フォームでも値が保持されるよう設計
