...
3. 承認済みの申請は、申請一覧画面の「承認済み」タブに表示される。
4. 承認後は、一般ユーザーは再度修正申請を行うことができる。
その場合、新規の stamp_correction_requests レコードを作成する。
...
- 勤怠の最終確定値は `attendances` テーブルに保持
- 修正申請の履歴は `stamp_correction_requests` テーブルに蓄積
...
attendance テーブルには承認後の最終確定データのみが反映されます。
そのため、一般ユーザーの勤怠一覧は承認後に勤怠が反映されます。
